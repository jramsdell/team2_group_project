...
public class TFIDF extends LabelPredictor {

    public TFIDF(IndexSearcher s) {
     super(s);
     searcher.setSimilarity(tfidf());

    }

    public SimilarityBase tfidf() {
        return new SimilarityBase() {
            ...
            protected float score(BasicStats basicStats, float freq, float docLen) {
                ...
                if (docLen == 0) {
                    ...
                    return (float)(Math.log((double)freq) * Math.log((double)basicStats.getNumberOfDocuments()  / basicStats.getDocFreq()));
                }
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        LabelPredictor predictor = new TFIDF(searcher);
        predictor.evaluate();
    }
